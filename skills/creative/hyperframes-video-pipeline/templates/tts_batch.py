#!/usr/bin/env python3
"""tts_batch.py — batch edge-tts narration for documentary scenes.

Usage: python tts_batch.py scenes.json outdir [--voice en-US-ChristopherNeural] [--rate +0%]

scenes.json: [{"id": "a1s1", "text": "...", "rate": "+0%", "voice": "..."}]
Output: outdir/<id>.mp3 for each scene + outdir/durations.json {"<id>": seconds}

Quirks baked in (do not "fix" these away):
- --rate=-12% MUST use the '=' form (argparse eats values starting with '-').
- subprocess uses sys.executable (bare 'python' can resolve to a different interpreter).
"""
import json, subprocess, sys, os

def main():
    manifest_path, outdir = sys.argv[1], sys.argv[2]
    voice = "en-US-ChristopherNeural"   # documentary narrator
    rate = "+0%"                        # ~116-158 wpm; -12% is too slow
