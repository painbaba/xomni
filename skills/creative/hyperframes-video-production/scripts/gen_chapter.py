...
Spec shape (see templates/chapter-spec.json for a full example):
{
  "id": "ch01", "title": "ACT ONE", "dur": 300.0, "music": "bed-tense",
  "fast": false,   # true = trailer pacing (scene dur = narration + 1.2s, min 3s)
  "scenes": [
    {"id":"a1s1","kind":"divider","eyebrow":"Act One","headline":"...","sub":"...",
     "narration":"a1s1","riser":true},           # riser SFX at scene start
    {"id":"a1s2","kind":"stat","stat":"1131","statLabel":"...","count":true,"boom":true},
    {"id":"a1s3","kind":"headline","masthead":"...","headline":"...","deck":"...","attr":"..."},
    {"id":"a1s4","kind":"quote","quote":"...","attr":"..."},
    {"id":"a1s5","kind":"timeline","nodes":[{"t":"May 2026","label":"..."}]},
    {"id":"a1s6","kind":"list","items":["...","..."]},
    {"id":"a1s7","kind":"endcard","credits":"..."},
    {"id":"a1s8","kind":"title","exact_dur":4.0}  # fixed duration, no narration
  ]
}
Kinds: divider, title, stat, quote, timeline, list, headline, endcard.

Verified behaviors baked in:
- Media srcs are PROJECT-ROOT-relative ("narration/x.mp3", "assets/beds/y.wav")
  even though the composition lives in compositions/ — lint fails otherwise.
- SFX each get their own track index (whoosh=12, riser=13, boom=14) so lint
  never flags overlaps; narration=10, music=11.
- Headline font auto-fits by plain-text length (layout check flags overflow).
- Count-up stats use Math.round(v).toLocaleString("en-IN") via a proxy tween.
- Scene durations derive from measured narration (durations.json), never spec
  guesses: docs narration+10s (min 40s), fast trailers narration+1.2s (min 3s).
"""
...
def hl_style(text):
    """auto-fit headline font size by plain-text length"""
    plain = re.sub(r"<[^>]+>", "", text)
    n = len(plain)
    if n <= 22: return ""
...
def build(spec_path, out_path, audio_dir):
    ...
    # resolve scene start/dur from narration durations (narration + breathing room)
    t = 0.0
    for s in spec["scenes"]:
        s["start"] = t
        if s.get("exact_dur"):
            s["dur"] = float(s["exact_dur"])
        elif s.get("narration") and s["narration"] in durs:
            if fast:
                s["dur"] = max(3.0, durs[s["narration"]] + 1.2)
            else:
                s["dur"] = max(40.0, durs[s["narration"]] + 10.0)
        else:
            s["dur"] = max(3.0 if fast else 40.0, s.get("dur", 45.0))
        t += s["dur"]
    chapter_len = t
    ...
    # audio: narration per scene (srcs are project-root-relative)
    audio_html = []
    for s in spec["scenes"]:
        nid = s.get("narration")
    ...
    # sfx — one track index per type so nothing collides
    for s in spec["scenes"]:
        if s.get("boom"):
