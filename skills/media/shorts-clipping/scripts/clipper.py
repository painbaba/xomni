#!/usr/bin/env python3
"""
AutoClipper — automatic podcast/stream clipping pipeline (verified working Aug 2026).
Usage:
  python clipper.py <youtube_url|local.mp4> [--clips N] [--window 45] [--min-score 2.5] [--keywords "money,tax,secret"] [--upload]

Pipeline:
  1. yt-dlp downloads best audio (or uses local file directly)
  2. faster-whisper transcribes with word timestamps (auto CUDA->CPU fallback)
...
Output: <workdir>/clips/clip_XX_<start>-<end>.mp4

Run with the Python that has yt_dlp installed. On this machine that is
Python 3.14 (NOT `python` on PATH which is 3.11):
...
def transcribe(audio):
    ...
    # IMPORTANT: driver present != cublas DLLs present. Wrap the whole transcribe call.
    devices = []
    try:
        import ctranslate2
...
def render(audio, words, clip, i):
    ...
    # 9:16 crop + scale. Use RELATIVE ass filename + cwd=WORKDIR to dodge
    # Windows drive-colon escaping in the subtitles filter.
...
def main():
    ...
    ap.add_argument('--upload', action='store_true', help='auto-upload clips to YouTube after rendering')
    ap.add_argument('--title-prefix', default='', help='prefix for auto-generated Shorts titles')
    ...
    for i, c in enumerate(clips, 1):
        o = render(audio, words, c, i)
        if o: outs.append(o)
    print(f'Done in {time.time()-t0:.0f}s. Clips in {os.path.join(WORKDIR, "clips")}')
    if args.upload and outs:
        import importlib.util
        spec = importlib.util.spec_from_file_location('yt_upload', os.path.join(WORKDIR, 'yt_upload.py'))
        mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
        svc = mod.get_authed_service()
        for o in outs:
            base = os.path.basename(o)
            mod.upload(svc, o, f'{args.title_prefix} {base}'.strip(), privacy=args.privacy)
