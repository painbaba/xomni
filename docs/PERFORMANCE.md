...
|---|-----------|-----------|--------|
| 1 | **prompt-enhancer auto mode** | One **extra `ctx.llm.complete` per turn** (~10–20s each on the slow provider) on top of every real turn | **Primary multiplier** — one LLM call per turn × 16 plugins' worth of context inflation |
...
Cold imports: all 16 plugins < 90ms one-time (`context-loader` 88ms worst on the after run;
